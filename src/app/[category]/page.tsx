...
import { categories, getCategoryNews, getSortedNews } from '../../lib/news';
...
export default async function CategoryPage({
  ...
  const { category } = await params;
  const data = getSortedNews(getCategoryNews(category));
  ...
  // secondaryArticles[0] and [1] → col2 (stacked small)
  // secondaryArticles[2]          → col3 (large right)
  const secondaryArticles = data.slice(1, 4);

  return (
    <main itemScope itemType="https://schema.org/CollectionPage" className="relative overflow-hidden">
      <Script
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[360px] bg-[radial-gradient(circle_at_top,rgba(219,230,243,0.34),transparent_68%)]" />
      <div className="relative mx-auto w-full max-w-[1380px] px-4 py-0 sm:px-6 md:px-8 md:py-8 lg:px-10">
        <CategoryHeader
          category={category}
          leadArticle={leadArticle}
          secondaryArticles={secondaryArticles}
        />
        <CategorySection data={data} category={category} />
        <ScrollToTopButton />
      </div>
    </main>
  );
