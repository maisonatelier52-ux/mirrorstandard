...
export default function HomeCategoryFront({ category, description, lead, secondary, tone = "ivory" }: Props) {
  if (!lead) return null;

  return (
    <section className="mt-8">
      {/* Section header — h2 for correct heading hierarchy */}
      <div className="mb-4 flex items-center gap-3">
        ...
        <Link
          href={`/${category}`}
          title={`More ${formatCategoryName(category)} news from Mirror Standard`}
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.6fr)]">
        ...
        >
          <div className="relative aspect-[21/9] overflow-hidden">
            <Image src={lead.image} alt={lead.title} fill className="object-cover transition-transform duration-700 group-hover:scale-[1.02]" sizes="(max-width: 1024px) 100vw, 50vw" />
          </div>
          {/* Lead title is h3 — under the h2 section heading above */}
          ...
          {secondary.map((story) => (
            ...
            >
              <div className="relative h-[68px] w-[88px] flex-shrink-0 overflow-hidden">
                <Image src={story.image} alt={story.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="88px" />
              </div>
